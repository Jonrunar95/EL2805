...
if __name__ == "__main__":
	...
	test_many = False
	if test_many:
		BUFFER_LENGTHS = [5000, 20000] # 5000 − 30000.
		LEARNING_RATES = [0.001, 0.0001] # 10^-3 - 10^-4
		HIDDEN_LAYERS = [1, 2]
		NEURON_PER_LAYERS = [16, 32, 68] # 8 - 128
		BATCH_SIZES = [8, 32] # 4 − 128
		DISCOUNT_FACTORS = [0.9, 0.99] # Value of the discount factor
		for bl in BUFFER_LENGTHS:
			for lr in LEARNING_RATES:
				for hl in HIDDEN_LAYERS:
					for npl in NEURON_PER_LAYERS:
						for bs in BATCH_SIZES:
							for df in DISCOUNT_FACTORS:
								print(bl, lr, hl, npl, bs, df)
								train(bl, lr, hl, npl, bs, df)
	else:
