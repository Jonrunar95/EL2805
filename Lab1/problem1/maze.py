...
class Maze:
            ...
            for j1 in range(self.maze.shape[1]):
				for i2 in range(self.maze.shape[0]):
            		for j2 in range(self.maze.shape[1]):
						if self.maze[i1,j1] != 1 and self.maze[i2, j2]:
    ...
    def __move(self, state, action):
		...
		minotaur_moves = __minotaur_move(state):
		for minotaur_moves in self.__minotaur_move(state):
	...
	def __minotaur_move(self, state):
		...
		for i, act in self.actions():
			# Minotaur can't stay in the same place
			if i == 0:
				continue
        	row = self.states[state][3] + act[0]
        	col = self.states[state][4] + act[1]
        	# Is the future position an impossible one ?
        	if(row == -1) or (row == self.maze.shape[0]) or (col == -1) or (col == self.maze.shape[1]):
				continue
            # Minotaur can walk through one set of walls
			elif(self.maze[row,col] == 1):
				row2 = self.states[state][0] + act[0]
        		col2 = self.states[state][1] + act[1]
				if(self.maze[row2,col2] == 0):
					possible_positions[i] = (row2, col2)
			else:
				possible_positions[i] = (row, col)
